...
export function toggleWrap(editor: EditorAPI, marker: string): boolean {
  ...
  if (selected.includes("\n")) {
    ...
    const allWrapped =
      nonEmpty.length > 0 &&
      nonEmpty.every((l) =>
        isLineWrapped(extractLineContent(l, marker).content, marker),
      );
    ...
    for (const line of lines) {
      if (line.trim() === "") {
        ...
        if (allWrapped) {
          newLines.push(prefix + unwrapLine(content, marker));
        } else {
          newLines.push(
            prefix + marker + stripAllMarkers(content, marker) + marker,
          );
        }
      }
    }
  ...
  if (before === marker || after === marker) {
    ...
    const lineEnd = lineEndNl === -1 ? doc.length : lineEndNl;
    const line = doc.slice(lineStart, lineEnd);

    const { prefix, content } = extractLineContent(line, marker);
    const allWrapped = isLineWrapped(content, marker);
